...
function createRuntimeConfig(options: CliOptions): RuntimeConfig {
...
function preflight(config: RuntimeConfig): void {
...
function cloneTemplate(options: CliOptions): void {
...
function createGithubRepo(options: CliOptions, config: RuntimeConfig): void {
...
function commitAndPush(config: RuntimeConfig): void {
  if (!gitRemoteExists(config.targetDir, 'origin')) {
...
function cloudflareAuth(config: RuntimeConfig): void {
  runInherited('pnpm', ['exec', 'wrangler', 'whoami'], config);
}

function createD1(config: RuntimeConfig): RuntimeConfig {
...
function createR2(config: RuntimeConfig): void {
  runInherited('pnpm', [
    'exec',
    'wrangler',
    'r2',
...
function createKV(config: RuntimeConfig): RuntimeConfig {
...
function writeWranglerConfig(config: RuntimeConfig): void {
...
function ensureEnvFiles(config: RuntimeConfig): void {
...
function updatePackageName(config: RuntimeConfig): void {
...
function readState(targetDir: string, fallbackConfig: RuntimeConfig): SetupState {
...
function writeState(targetDir: string, state: SetupState): SetupState {
...
function runInheritedRaw(command: string, args: string[], cwd: string): void {
...
function runQuiet(command: string, args: string[], cwd: string): void {
...
function execVersion(command: string): string {
...
function readWranglerConfig(wranglerPath: string): WranglerConfig {
...
function bufferToString(value: Buffer | null): string {
